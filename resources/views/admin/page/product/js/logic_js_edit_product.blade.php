...
<script>
    const productTargetData = @json($product);
    const productTargetSpecData = @json($spec);
    ...
    $("#add-product-post-form").submit(function (e) {
        ...
        form_data.append('id', productTargetData.id);
    ...
    var loadFile = function(event) {
        ...
        if (fileInput.files.length > 0) {
            // Nếu có file mới, hiển thị ảnh mới
    ...
    function buildAttributeForm(data) {
        ...
        for (var i = 0; i < attributes.length; i++) {
            ...
            for (var j = 0; j < attributes[i].spec.length; j++) {
                ...
                for (var g = 0; g < productTargetSpecData.length; g++) {
                    if (attributes[i].spec[j].id == productTargetSpecData[g].id) {
                        inputRadioSpec.checked = "checked";
                    }
                }
    ...
    function buildStatusForm (data) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            if (data[i].id == productTargetData.status_id) {
                inputRadioStatus.checked = 'checked';
            }
    ...
    function buildCategoryForm (data) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            if (data[i].id == productTargetData.category_id) {
    ...
    function buildProducerForm (data) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            option.value = data[i].id;
            option.textContent = data[i].name
            if (data[i].id == productTargetData.producer_id) {
                option.selected = 'selected';
            }
            selectProducerInput.appendChild(option);
        }
        initProducerInput(selectProducerInput);
        divContent.appendChild(labelSelect);
    ...
    function buildModelForm(data, producers, producerId) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            if (data[i].id == productTargetData.model_id) {
                option.selected = 'selected';
            }
        }
    ...
    $(document).ready(function() {
        ...
        // Hiển thị ảnh đã có trong gallery khi tải trang
        $('#gallery-container .image-container').each(function() {
            const img = $(this).find('img').attr('src');
            const removeButton = $(this).find('.remove-image');

            removeButton.on('click', function() {
                $(this).parent().remove();
                // Thực hiện xóa ảnh khỏi mảng filesArray hoặc backend nếu cần
            });
        });
        ...
        function setGalleryImage(filesArray) {
